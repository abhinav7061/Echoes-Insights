...
const BlogActions = ({ blogId, authorId, authorName, showName = true, liked, saved, followed, likesCount, setLiked, setSaved, setFollowed, setLikesCount, className, iconClassName, btnClassName, shareBtnsClassName = 'md:-left-16', otherComponent, showReportBtn = true }) => {
    return (
        <div className={`flex gap-2 ${className}`}>
            ...
            <CopyBtn showName={showName} btnClassName={`w-full ${btnClassName}`} copy={`${window.location.protocol}//${window.location.host}/blog/${blogId}`} className={`w-full ${iconClassName}`} />
            <BlogLikeDislike blogId={blogId} showName={showName} className={`w-full ${iconClassName}`} btnClassName={`w-full ${btnClassName}`} liked={liked} likesCount={likesCount} setLiked={setLiked} setLikesCount={setLikesCount} />
