...
const ReadingHistory = ({ shortFetch, heading, emptyScreen = null }) => {
    const perPage = 8;
    ...
    const {
        ...
        loaderRef,
        hasMore
    } = useInfiniteApi('/history', null, { params: { limit: perPage } });
    if (!blogs.length && !loading) return emptyScreen;
    return (
        <div className='mb-4'>
            <h1 className="text-xl sm:text-2xl font-bold mb-4">{heading}</h1>
            <div className={cn('gap-[2px] xs:gap-1.5 sm:gap-3 w-full relative',
